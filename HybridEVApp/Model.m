function Model()
% RUNEVSCRIPT
% This script builds and runs the HybridEVModel with live weather API integration.
% The API call is now performed in the main script and the data is
% assigned to the base workspace for Simulink to access.
% Simply type `runEV` into the command window and press enter to start.
    
    % Call the run function which now handles fetching and simulation
    runHybridEVModel();
...
% Fetches live weather data, runs the simulation, and creates plots.
    ...
    try
        % FIX: The API key below is a placeholder. You MUST replace this with a valid key.
        apiKey = '0a6b0fb4875fb59895401d14b2034fc0'; 
        lat = 50.2667;
        lon = -5.0527;
        ...
        % FIX: Time step of OpenWeatherMap forecast is 3 hours (10800 seconds).
        % We need to use this to correctly scale the time series.
        time_step_s = 3 * 3600; 
        ...
        disp(ME.message);
        n = 40;
        % FIX: Use the correct 3-hour time step for the fallback data as well
        time_step_s = 3 * 3600;
        time_points_api = 0:time_step_s:time_step_s*(n-1);
    ...
    % FIX: Pass the modelName as the first argument
    buildSimulinkModel(modelName, sim_stop_time);
    ...
    % FIX: The old check for ScopeData is no longer needed.
    
    % Extract time and data from structures.
    % FIX: The function now returns empty arrays for missing data to prevent errors.
    function [t,y] = extractTimeData(S)
        % FIX: Check if the output is a timeseries object
        if isa(S, 'timeseries') && ~isempty(S.Data)
            t = S.Time;
            y = S.Data;
        else
            t = []; y = []; % Return empty arrays for trapz to handle gracefully
        end
    end
    ...
    grid on;
    xlabel('Time (hours)');
    % FIX: Add a static y-axis limit to keep the plot's scale consistent
    ylim([0 1500]);
    ...
    subplot(4, 2, 8);
    % FIX: Check if t is empty before calling trapz to prevent 'X must be a vector' error
    if ~isempty(t)
        total_renewable = trapz(t, renewable_gen) / 3600; % Convert to kWh
    ...
    % FIX: Check for empty renewable_share array before calculating mean
    ...
    % FIX: Check if the soc array is not empty before accessing the last element
    if ~isempty(soc)
    ...
    % FIX: Check for empty tariff array before calculating mean
    if ~isempty(tariff)
...
function buildSimulinkModel(modelName, stopTime)
% BUILDSIMULINKMODEL
% Creates the Simulink model with data inputs from the base workspace.
    ...
    % Controller
    % --------------------
    % FIX: The block type MUST be 'MATLAB Function'
    ...
    % Set the script on the MATLAB Function block
    controllerCode = strjoin({
        ...
        'Pbat_power = 0; % Placeholder, actual SOC change is handled internally'
        ''
    ...
    set_param(ctrlPath, 'Script', controllerCode);
...
% Creates the Simulink model with data inputs from the base workspace.
    ...
    % Clean up any existing model with the same name
    if bdIsLoaded(modelName)
        close_system(modelName, 0);
    end
    ...
    % Controller
    % --------------------
    % FIX: Corrected block type to 'MATLAB Function'
    ctrlPath = [modelName '/Controller'];
    add_block('simulink/User-Defined Functions/MATLAB Function', ctrlPath, ...
        'Position', [x0+200+100 y0-10 x0+200+240+100 y0+3*dy+60]);
