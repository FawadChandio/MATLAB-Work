classdef TransformerDiagnosticApp4< matlab.apps.AppBase
  ...
  function manualPredictCallback(app, fig, gasFields, tempField, loadField)
    ...
    try
        ...
        % Define hardcoded default values for optional gases
        defaultValues = struct(...
            'co', 50, ...    % Default CO value (ppm)
            'co2', 360, ...  % Default CO2 value (ppm)
            'h2o', 25 ...     % Default H2O value (ppm)
        );
        
        % Handle optional gases - add if missing or fill NaN values
        ...
        for i = 1:length(optionalCols)
            ...
            if ~ismember(gas, data.Properties.VariableNames)
                ...
                data.(gas) = repmat(defaultValues.(gas), height(data), 1);
                app.updateStatus(sprintf('Added missing %s column with default value %.1f ppm', gas, defaultValues.(gas)), 'yellow');
            else
                % Fill NaN values with default
                nanIndices = isnan(data.(gas));
                if any(nanIndices)
                    data.(gas)(nanIndices) = defaultValues.(gas);
                    app.updateStatus(sprintf('Replaced %d NaN values in %s with default %.1f ppm', sum(nanIndices), gas, defaultValues.(gas)), 'yellow');
                end
            end
        end
        
        % Clean data - replace invalid values with median (for required columns only)
    ...
    try
        ...
        % ============ GAS ANALYSIS ============
       % In the generateReport method, update the gas analysis section:
                    % ============ GAS ANALYSIS ============
                    ...
                    if ~isempty(app.TransformerData)
                        % All gases to display (including CO2 and H2O)
                        gases = {
                            'H₂',   'Hydrogen';
                            'CH₄',  'Methane';
                            'C₂H₆', 'Ethane';
        ...
        if isfield(app.TrainedModels, regexprep(app.ModelSelectionDropDown.Value, '\s*\(Built-in\)', ''))
            model = app.TrainedModels.(regexprep(app.ModelSelectionDropDown.Value, '\s*\(Built-in\)', ''));
            perfText = {
   ...
   function updateDuvalTriangle(app)
    ...
    % All possible variables including optional gases
    allVars = {'h2','ch4','c2h6','c2h4','c2h2','co','co2','h2o','temperature','load'};
    prettyNames = {'H₂','CH₄','C₂H₆','C₂H₄','C₂H₂','CO','CO₂','H₂O','Temp','Load'};
    
    % Find which variables actually exist in the data (should have all now)
