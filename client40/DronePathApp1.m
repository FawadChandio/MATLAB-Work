classdef DronePathApp1 < matlab.apps.AppBase
    ...
    properties (Access = public)
        ...
        UIAxes                    matlab.ui.control.UIAxes  % Make sure this is properly defined
    end

    % Animation variables and control properties
    properties (Access = private)
        xPath                     % X-coordinates for drone path
        yPath                     % Y-coordinates for drone path
        zPath                     % Z-coordinates for drone path
        droneImage                % Image object for drone icon
        trackLine                 % Line object for original red path
        completedPathLine         % Line object for blue track path
        trackColor = [1, 0, 0]    % Default path color (red)
        isRunning = false         % Control flag for start/stop functionality
    end
    ...
    methods (Access = private)
        ...
        function plotPath(app)
            ...
            if isempty(app.xPath) || isempty(app.yPath) || isempty(app.zPath)
                return;  % Skip plotting if no waypoints are loaded
            end

            % Create a new axes handle if UIAxes does not exist
            if isempty(app.UIAxes) || ~isgraphics(app.UIAxes)
                app.UIAxes = axes(app.UIFigure, 'Position', [0.3 0.2 0.65 0.6]);
            end
            ...
            % Plot the planned path in the specified color
            app.trackLine = plot3(app.UIAxes, app.xPath, app.yPath, app.zPath, ...
                                  'Color', app.trackColor, 'LineWidth', 2);
            hold(app.UIAxes, 'on');
            ...
            app.droneImage = imagesc(app.UIAxes, 'XData', [app.xPath(1)-0.1, app.xPath(1)+0.1], ...
                                     'YData', [app.yPath(1)-0.1, app.yPath(1)+0.1], 'CData', droneIcon);
            ...
            view(app.UIAxes, 3);  % Set a 3D view
            grid(app.UIAxes, 'on');
            hold(app.UIAxes, 'off');
        end
        ...
        function animateDrone(app)
            app.isRunning = true;  % Start animation
            midPointIdx = ceil(length(app.xPath) / 2);  % Index for hover
            speed = 0.05;           % Set lower for smoother movement
            hoverDuration = 2;      % Hover time at midpoint
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.HexColorField = uieditfield(app.UIFigure, 'text', 'Position', [170 540 150 22]);
            app.HexColorField.ValueChangedFcn = createCallbackFcn(app, @HexColorFieldValueChanged, true);

            % Waypoints table
            app.WaypointsTable = uitable(app.UIFigure, 'Position', [20 350 250 150]);
            app.WaypointsTable.ColumnName = {'X-Coordinate', 'Y-Coordinate'};
            app.WaypointsTable.ColumnEditable = [true, true];
            ...
            app.SaveWaypointsButton = uibutton(app.UIFigure, 'push', 'Position', [150 260 120 30], 'Text', 'Save Waypoints');
            app.SaveWaypointsButton.ButtonPushedFcn = createCallbackFcn(app, @SaveWaypointsButtonPushed, true);

            app.ClearButton = uibutton(app.UIFigure, 'push', 'Position', [280 260 120 30], 'Text', 'Clear');
            app.ClearButton.ButtonPushedFcn = createCallbackFcn(app, @ClearButtonPushed, true);

            % Start/stop buttons
            app.StartButton = uibutton(app.UIFigure, 'push', 'Position', [20 100 120 30], 'Text', 'Start');
            app.StartButton.ButtonPushedFcn = createCallbackFcn(app, @StartButtonPushed, true);
            
            app.StopButton = uibutton(app.UIFigure, 'push', 'Position', [150 100 120 30], 'Text', 'Stop');
            app.StopButton.ButtonPushedFcn = createCallbackFcn(app, @StopButtonPushed, true);

            % UIAxes for drone path
            app.UIAxes = axes(app.UIFigure, 'Position', [0.3 0.2 0.65 0.6]);
            axis(app.UIAxes, 'equal');
        end
    end
    ...
    methods (Access = public)
        ...
        function app = DronePathApp
            % Create UI components and set up initial state
