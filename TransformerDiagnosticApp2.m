classdef TransformerDiagnosticApp2 < matlab.apps.AppBase
    ...
    try
        ...
        port = "COM3";  % Your Arduino port (confirm in Device Manager)
        app.updateStatus(['Trying to connect to ' port '...'], 'yellow');

        % Try connecting
        app.SerialObj = serialport(port, 9600);
        configureTerminator(app.SerialObj, "LF");
        app.SerialObj.Timeout = 5;

        % Flush any old data
        flush(app.SerialObj);

        % Send handshake
        pause(2);  % Wait 2 seconds for Arduino to get ready
        writeline(app.SerialObj, "TEST");

        % Read response
        response = readline(app.SerialObj);
        disp("Handshake response: " + response);  % Debug line

        if contains(response, "ARDUINO")
            % Connected
            configureCallback(app.SerialObj, "terminator", @(src, ~)app.readLiveData(src));
            app.updateStatus(['Connected to live data on ' port], 'green');
            app.LiveDataButton.Text = 'Disconnect Live Data';
            app.LiveDataButton.BackgroundColor = [0.85, 0.33, 0.1];
        else
            delete(app.SerialObj);
            app.SerialObj = [];
            error('Invalid handshake. Expected "ARDUINO", got: %s', response);
        end
        ...
        app.updateStatus(['Live data error: ' ME.message], 'red');
            ...
            try
                if ~isempty(app.SerialObj)
                    if isvalid(app.SerialObj)
                        % Disable callback and delete port
                        configureCallback(app.SerialObj, "off");
                        delete(app.SerialObj);
                    end
                    app.SerialObj = [];  % Clear reference
                end
        
                % Update UI
                app.LiveDataButton.Text = 'Connect Live Data';
                app.LiveDataButton.BackgroundColor = [0.47, 0.67, 0.19]; % Green
        ...
        try
            ...
            % Parse the data (expected format: H2:val,CH4:val,...,LOAD:val)
            try
                ...
                % Create table with all 11 fields (Time + 10 values)
                newData = table(datetime('now'), NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, NaN, ...
                    'VariableNames', {'Time', 'h2', 'ch4', 'c2h6', 'c2h4', 'c2h2', 'co', 'co2', 'h2o', 'temperature', 'load'});
                ...
                % Update UI table and visualizations
                app.DataGrid.Data = app.TransformerData;
                app.updateVisualizations();
    
                % Update AI Diagnosis text
                app.AIDiagnosisTextArea.Value = {};
                for g = {'h2', 'ch4', 'c2h6', 'c2h4', 'c2h2', 'co', 'co2', 'h2o'}
                    gas = g{1};
                    value = newData.(gas);
                    [~, interpretation] = app.getGasInterpretation(gas, value);
                    line = sprintf('%s = %.2f ppm → %s', upper(gas), value, interpretation);
                    app.AIDiagnosisTextArea.Value{end+1} = line;
                end
    
                % Enable buttons after receiving first row
    ...
    % All desired gas + env variables
    allVars = {'h2','ch4','c2h6','c2h4','c2h2','co','co2','h2o','temperature','load'};
    prettyNames = {'H₂','CH₄','C₂H₆','C₂H₄','C₂H₂','CO','CO₂','H₂O','Temp','Load'};

    % Find which ones are present in data
    presentMask = ismember(allVars, data.Properties.VariableNames);
    availableVars = allVars(presentMask);
    displayNames = prettyNames(presentMask);
    ...
    validCols = ~all(isnan(dataArray), 1) & std(dataArray, 0, 1) > 0;
    dataArray = dataArray(:, validCols);
    availableVars = availableVars(validCols);
    displayNames = displayNames(validCols);

    if numel(availableVars) < 2
        cla(app.CorrelationHeatmapAxes);
    ...
    corrMatrix(isnan(corrMatrix)) = 0; % fallback
    ...
    app.CorrelationHeatmapAxes.XTick = 1:length(displayNames);
    app.CorrelationHeatmapAxes.YTick = 1:length(displayNames);
